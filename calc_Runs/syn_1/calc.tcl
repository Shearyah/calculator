...
set rc [catch {
  open_project {calc.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {top}
  export_db {calc_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc
  optimize_rtl
  report_area -file calc_rtl.area
  export_db {calc_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea calc_gate.area
  legalize_phy_inst
  export_db {calc_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
