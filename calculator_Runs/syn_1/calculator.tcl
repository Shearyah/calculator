...
set rc [catch {
  open_project {calculator.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {top}
  export_db {calculator_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../calculator.adc
  optimize_rtl
  report_area -file calculator_rtl.area
  export_db {calculator_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea calculator_gate.area
  legalize_phy_inst
  export_db {calculator_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
